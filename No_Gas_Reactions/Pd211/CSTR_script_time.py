...
#     )
    species_path = (
        os.path.dirname(os.path.abspath(__file__))
        + f"/results/time/{git_file_string}/species_pictures"
    )
    print(species_path)
    results_path = (
        os.path.dirname(os.path.abspath(__file__))
       + f"/results/time/{git_file_string}/{reactor_type_str}/{temp}/results"
    )
    logging.warning(f"Saving results in {results_path}, the file's name is _temp_{temp}_volume_flows_{volume_flow_str}.csv")
    flux_path = (
        os.path.dirname(os.path.abspath(__file__))
        + f"/results/time/{git_file_string}/{reactor_type_str}/{temp}/flux_diagrams"
    )
...
#     )
    output_filename = (
        results_path
        + f"/Spinning_basket_area_{cat_area_str}_energy_{energy}"
        + f"_temp_{temp}_volume_flows_{volume_flow_str}.csv"
    )
...
Temps = [500]
Pressures = [1] # 1 bar
#volume_flows = np.logspace(-11,2,100) #for 1000K
volume_flows = np.logspace(-13,7,300)  #for 500K
